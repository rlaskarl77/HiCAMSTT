...
class WorldTrackModel(pl.LightningModule):
    ...
    def draw_detection(self, item, output, batch_idx=0):
        ...
        mosaic = np.concatenate([np.concatenate([rgb_cams[0], rgb_cams[1], rgb_cams[2]], axis=1),
                                 np.concatenate([mixed[0], mixed[1], mixed[2]], axis=1)], axis=0)
        mosaic = (mosaic * 255).astype(np.uint8)
        mosaic = cv2.cvtColor(mosaic, cv2.COLOR_RGB2BGR)
        mosaic_resized = cv2.resize(mosaic, (1280, 720))
        cv2.imshow('Detection', mosaic_resized)
