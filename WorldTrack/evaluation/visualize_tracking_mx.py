...
SAVE_PATH = '/home/namgi/TrackTacular/visualization/multiviewx/demo'
PRED_FILE = '/home/namgi/TrackTacular/WorldTrack/lightning_logs/version_10/mota_pred.txt'
...
#     intrinsic_camera_path = os.path.join(DATA_PATH, 'calibrations', 'intrinsic_zero')
...
#     extrinsic_params_file_root = ET.parse(os.path.join(DATA_PATH, 'calibrations', 'extrinsic',
...
def get_intrinsic_extrinsic_matrix(camera_i):
    intrinsic_camera_path = os.path.join(DATA_PATH, 'calibrations', 'intrinsic')
    ...
    extrinsic_camera_path = os.path.join(DATA_PATH, 'calibrations', 'extrinsic')
...
def project_2d_points(intrinsic_mat, extrinsic_mat, input_points, x_offset, y_offset, z_offset):
    ...
    input_points[0, :] = input_points[0, :] * 0.025 + x_offset
    input_points[1, :] = input_points[1, :] * 0.025 + y_offset
...
def plot(path):
    ...
    # camera_ids = {0: 'C1', 1: 'C2', 2: 'C3', 3: 'C4', 4: 'C5', 5: 'C6', 6: 'C7'}a
    camera_ids = {0: 'C1', 1: 'C2', 2: 'C3', 3: 'C4', 4: 'C5', 5: 'C6'}
    ...
    image_paths = {camera_id: sorted([osp.join(DATA_PATH, f'Image_subsets/{camera_ids[camera_id]}/{int(frame):04d}.png') for frame in frames]) for camera_id in camera_ids.keys()}
    ...
    for frame_idx, frame in enumerate(frames):
        ...
        w_size, w_offset = compute_size((1920//3*2, 1080//3), w_image.size)
        w_image = w_image.resize(w_size)
        mosaic.paste(w_image, (w_offset[0]+1920//3, w_offset[1]+1080//3*2))
        ...
        for cam_id in camera_ids.keys():
            ...
            mosaic.paste(image.resize((1920//3, 1080//3)), (1920//3*(cam_id%3), 1080//3*(cam_id//3)))
        
        mosaic.save(frame_save_path)
    
    os.system(f'ffmpeg -framerate 2 -pattern_type glob -i "{SAVE_PATH}/*.png"  {SAVE_PATH}/result.mp4')
