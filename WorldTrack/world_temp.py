...
class WorldTrackModel(pl.LightningModule):
    def __init__(
        ...
        self.feat_correct_prev, self.feat_correct_random = 0, 0
        self.feat_total_prev, self.feat_total_random = 0, 0
        
        self.feat_dist_prev = torch.full((self.max_features, self.max_features), -1, device=self.device, dtype=torch.float32)
        self.feat_dist_random = torch.full((self.max_features, self.max_features), -1, device=self.device, dtype=torch.float32)
        
        self.feat_dist_count_prev = torch.zeros(self.max_features, self.max_features, device=self.device)
        self.feat_dist_count_random = torch.zeros(self.max_features, self.max_features, device=self.device)
        
        self.feat_dist_prev_list = []
    ...
    def compute_bev(self, item):
        ...
        bev_raw = output['bev_raw'].clone().detach()
        bev_feat = output['bev_feat'].clone().detach()

        output_random_prev = self.model(
        ...
        bev_prev_random_raw = output_random_prev['bev_raw'].clone().detach()
        ...
        bev_random_raw = output_random['bev_raw'].clone().detach()
        bev_random_feat = output_random['bev_feat'].clone().detach()
        
        return bev_prev_raw, bev_raw, bev_random_raw, bev_feat, bev_random_feat 
    ...
    def check_distance(self, item, target):
        bev_prev_raw, bev_raw, bev_random_raw, bev_feat, bev_random_feat  = \
            self.compute_bev(item)
        
        # get bev features with pid
        
        feat_prev_raw, pid_prev, unique_pid_prev = self.get_feature_vec_with_pid(bev_prev_raw, target['pid_bev_prev'])
        feat_raw, pid, unique_pid = self.get_feature_vec_with_pid(bev_raw, target['pid_bev'])
        feat_random_raw, pid_random, unique_pid_random = self.get_feature_vec_with_pid(bev_random_raw, target['pid_bev_rand'])
        
        # print(bev_raw.shape, bev_feat.shape, pid.shape, pid_prev.shape, pid_random.shape)
        feat, _, _ = self.get_feature_vec_with_pid(bev_feat, target['pid_bev'])
        feat_random, _, _ = self.get_feature_vec_with_pid(bev_random_feat, target['pid_bev_rand'])
        
        # # find matching pids
        # matching_pid_with_prev = []
        # matching_pid_with_random = []
        # for pid in unique_pid:
        #     if pid in unique_pid_prev:
        #         matching_pid_with_prev.append(pid)
        #     if pid in unique_pid_random:
        #         matching_pid_with_random.append(pid)
        # collect features
        dist_prev = 1 - F.cosine_similarity(feat_raw.unsqueeze(1), feat_prev_raw.unsqueeze(0), dim=2)
        dist_random = 1 - F.cosine_similarity(feat_raw.unsqueeze(1), feat_random_raw.unsqueeze(0), dim=2)
        dist_feat = 1 - F.cosine_similarity(feat.unsqueeze(1), feat_random.unsqueeze(0), dim=2)
        
        # print(dist_prev.shape, dist_random.shape, dist_feat.shape)
        # print(pid.shape, pid_prev.shape, pid_random.shape)
        
        return dist_prev, dist_random, dist_feat, pid, pid_prev, pid_random
    ...
    def test_step(self, batch, batch_idx):
        ...
        dist_prev, dist_random, dist_feat, pid, pid_prev, pid_random = \
            self.check_distance(item, target)
            
        for i, p in enumerate(pid):
            
            for j, p_prev in enumerate(pid_prev):
                
                if self.feat_dist_count_prev[p, p_prev] == 0:
                    self.feat_dist_prev[p, p_prev] = dist_prev[i, j]
                else:
                    self.feat_dist_prev[p, p_prev] = \
                        (self.feat_dist_prev[p, p_prev] * \
                        self.feat_dist_count_prev[p, p_prev] + dist_prev[i, j]) / \
                        (self.feat_dist_count_prev[p, p_prev] + 1)
                
                self.feat_dist_count_prev[p, p_prev] += 1
                
            # print(dist_prev.shape, pid_prev.shape)
            
            min_p_prev = pid_prev[torch.argmin(dist_prev[i])]
            
            if p == min_p_prev:
                self.feat_correct_prev += 1
            self.feat_total_prev += 1
            ...
            for j, p_feat in enumerate(pid_random):
                
                if self.feat_dist_count[p, p_feat] == 0:
                    self.feat_dist[p, p_feat] = dist_feat[i, j]
                else:
                    self.feat_dist[p, p_feat] = \
                        (self.feat_dist[p, p_feat] * \
                        self.feat_dist_count[p, p_feat] + dist_feat[i, j]) / \
                        (self.feat_dist_count[p, p_feat] + 1)
                
                self.feat_dist_count[p, p_feat] += 1
            
            min_p_feat = pid_random[torch.argmin(dist_feat[i])]
            if p == min_p_feat:
                self.feat_correct += 1
            self.feat_total += 1
    ...
    def on_test_epoch_end(self):
        ...
        acc_prev = self.feat_correct_prev / self.feat_total_prev
        acc_random = self.feat_correct_random / self.feat_total_random
        acc_feat = self.feat_correct / self.feat_total
        
        self.log('acc_prev', acc_prev)
        self.log('acc_random', acc_random)
        self.log('acc_feat', acc_feat)
        
        # print(f'prev: {acc_prev}, random: {acc_random}, feat: {acc_feat}')
        # clip pids
        count_sum = self.feat_dist_count_prev.sum(1) + self.feat_dist_count_random.sum(1) \
            + self.feat_dist_count.sum(1)
        
        max_pid = torch.argmin(count_sum)
        # print(count_sum, max_pid)
        
        self.feat_dist_prev = self.feat_dist_prev[:max_pid, :max_pid]
        self.feat_dist_random = self.feat_dist_random[:max_pid, :max_pid]
        self.feat_dist = self.feat_dist[:max_pid, :max_pid]
        
        # save plots to tensorboard in eval loop
        
        # set dist to 2 if not calculated
        self.feat_dist_prev[self.feat_dist_prev == -1] = 2
        self.feat_dist_random[self.feat_dist_random == -1] = 2
        self.feat_dist[self.feat_dist == -1] = 2
        
        # normalize to 0 to 1
        self.feat_dist_prev /= 2.
        self.feat_dist_random /= 2.
        self.feat_dist /= 2.
        
        # to numpy
        dist_prev = self.feat_dist_prev.detach().cpu().numpy()
        dist_random = self.feat_dist_random.detach().cpu().numpy()
        dist_feat = self.feat_dist.detach().cpu().numpy()
        
        writer = self.logger.experiment
        fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(20, 8))
        ax1.imshow(dist_prev)
        ax2.imshow(dist_random)
        ax3.imshow(dist_feat)
